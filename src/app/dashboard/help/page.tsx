  ...
  return (
    ...
    >
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-2">
        <div>
          <h1 className="text-2xl md:text-3xl font-inter-semibold text-[#04B851] drop-shadow-sm flex items-center gap-2">
            <BsQuestionCircle className="text-[#04B851]" /> Help & Support
          </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <motion.div
          ...
          className="bg-[#FFFFFF] rounded-2xl shadow-lg p-6 border-2 border-[#E0E0E0] flex flex-col gap-4"
        >
          <h2 className="text-xl font-inter-semibold text-[#04B851] mb-2 flex items-center gap-2">
            <BsBook /> FAQs
          </h2>
          <div className="divide-y divide-[#E0E0E0]">
            {faqs.map((faq, idx) => (
              <details key={idx} className="py-3 group cursor-pointer">
                <summary className="font-semibold text-[#1A1A1A] flex items-center justify-between text-base group-open:text-[#04B851] transition">
                  {faq.q}
                  <span className="ml-2 text-[#04B851]">+</span>
                </summary>
                <p className="text-[#4D4D4D] mt-2 text-sm pl-2">{faq.a}</p>
              </details>
            ))}
          </div>
        </motion.div>
