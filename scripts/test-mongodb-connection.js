 ...
 * Run with: node --experimental-modules scripts/test-mongodb-connection.mjs
 */
...
dns.resolve(hostname, (err, addresses) => {
  if (err) {
    ...
    if (MONGO_URI.includes('mongodb+srv')) {
      ...
      dns.resolveSrv(`_mongodb._tcp.${hostname}`, (errSrv, records) => {
        if (errSrv) {
          console.error(
      ...
      // Skip SRV test for standard MongoDB connections
      testMongoDBConnection()
    }
  }
})
  ...
  mongoose
    ...
    .catch((err) => {
      ...
      if (err.name === 'MongoServerSelectionError') {
        ...
        console.log(
          '3. Verify the hostname in your connection string is correct',
        )
      } else {
